...
def read_charset(charset_fp):#读取字符集文件
    alphabet = []
...
def read_tsv_file(fp, sep='\t', img_folder=None, mode='eval'):#读取tsv文件，包括路径、分隔符、图片文件夹、模式
...
def read_img(path: Union[str, Path], gray=True) -> np.ndarray:#读取图片文件并转换为一个ndarray数组，并转换为灰度图读取
    """
...
def save_img(img: Union[Tensor, np.ndarray], path):#将图片以ndarray数组和张量形式保存到路径
